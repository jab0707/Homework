function [CV, dVdtMax,TMP] = main_LRd_strand(p, varargin)
% /***************************************************************************
...
% APL
% dVdt and CV calculations
if nargin >1 && varargin{1} > 1
    dV = diff(TMP,1,1);
    dVdt = dV/t(2); % the second time point in t is equal to the timestep
    [dVdtmax, tmax] = max(dVdt,[],1);
    tmax = t(tmax);
    CV = (x(35)-x(15))/(tmax(35)-tmax(15))*1000; % cm/s
    dVdtMax = mean(dVdtmax(20:30));
    disp(['CV: ',num2str(CV),' cm/s'])
    disp(['dVdtMax: ',num2str(dVdtMax),' V/s'])
else
    CV = -1;
    dVdtMax = -1;
end
