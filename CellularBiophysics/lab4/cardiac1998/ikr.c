...
#define nrn_init _nrn_init__IKr
#define _nrn_initial _nrn_initial__IKr
#define nrn_cur _nrn_cur__IKr
#define _nrn_current _nrn_current__IKr
#define nrn_jacob _nrn_jacob__IKr
#define nrn_state _nrn_state__IKr
#define _net_receive _net_receive__IKr 
#define rate rate__IKr 
#define states states__IKr 
...
#define mtau mtau_IKr
 double mtau = 0;
#define minf minf_IKr
 double minf = 0;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 IKr C:/Users/Jake/Documents/GitHub/Homework/Homework/CellularBiophysics/lab4/cardiac1998/ikr.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "Cardiac IKr current";
...
double alp (  double _lv ) {
 ...
 _lv = _lv ;
   ...
   _lalp = _lq10 * 0.0003 * ( _lv + 14.1 ) / ( 1.0 - exp ( - ( _lv + 14.1 ) / 5.0 ) ) ;
...
double bet (  double _lv ) {
 ...
 _lv = _lv ;
   _lq10 = pow( 3.0 , ( ( celsius - 37.0 ) / 10.0 ) ) ;
   _lbet = _lq10 * 7.3898e-5 * ( _lv - 3.3328 ) / ( exp ( ( _lv - 3.3328 ) / 5.1237 ) - 1.0 ) ;
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
   ik = gKr / ( 1.0 + exp ( ( v + 15.0 ) / 22.4 ) ) * m * ( v - ek ) ;
   }
 _current += ik;
...
_deriv1_advance = 0;
 if(error){fprintf(stderr,"at line 44 in file ikr.mod:\n	SOLVE states METHOD derivimplicit\n"); nrn_complain(_p); abort_run(error);}
    if (secondorder) {
    int _i;
    for (_i = 0; _i < 1; ++_i) {
