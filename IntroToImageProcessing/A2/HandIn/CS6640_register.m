...
%
%Notes:
%   in cpts it was not specified which column was to be the x and which was
%   to be the y coordinates in the image of the correspondence poinst. So I
%   chose the first column to be x and the second to be y.
...
%Initilize the quadratic matricies to solve the equations
...
q = inv(ptMatrix)*transPtMatrix;%Calculate the quadratic coeffs
...
%%%%%%%%%%%%%%%%%%%%%%%%%
%Now we have both, we need to apply one of them according to the switch
%%%%%%%%%%%%%%%%%%%%%%%%%

%Initilize the new points, old points, and old gray values for
%mapping/interpolation on the new corrdinate system
newX = zeros(size(im(:)));
newY = zeros(size(im(:)));
oldX = zeros(size(im(:)));
oldY =zeros(size(im(:)));
...
%%%%
%Now we need to re map the values from the new positions back into the
%image grid. To do so we use griddata
%%%%
    

remappedData = griddata(oldX,oldY,origGrayValue,newX,newY);

%Any nans were values that fell outside of the mapping domain and should be
%changed to 0 for visualization
nanIdx = find(isnan(remappedData));
for nId = 1:length(nanIdx)
    remappedData(nanIdx(nId)) = 0;
end

%Now for each pixel in the registered image, assign its value according to
%the interpolation
imr = zeros(size(im));
counter = 1;
for r = 1:size(imr,1)
